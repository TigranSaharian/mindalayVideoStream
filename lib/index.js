...
const API_URL_KEY = 'https://localhost:44320/api';
var connection = null;

export class MindalayVideoConnection{
    constructor(options){
        this.options = options;
        if(options.partnerId && options.secretKey){
            this.checkPartnerStatus(options.partnerId, options.secretKey).then(response => {
                if(response.data){
                    this.InitConnection(options);
                }else{
                    console.log(response);
                    // alert("please become an partner >> mindalay.com/becomeanpartner");
                }
                return;
    ...
    checkPartnerStatus(partnerId, secretKey) { 
        return new Promise((resolve, reject) => {
            fetch(`${API_URL_KEY}/api/partner/checkpartnerstatus`, {
                type: 'GET',
                headers: {
                    'partnerId': partnerId,
                    'secretKey': secretKey
